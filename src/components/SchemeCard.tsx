...
export default function SchemeCard({ item, onOpen, rank }: Props) {
  ...
  return (
    <div className="group flex flex-col rounded-3xl border border-ink/10 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="mt-5 flex items-center justify-between border-t border-ink/10 pt-4">
        <span className="text-xs font-bold text-ink/70">{scheme.amountLabel}</span>
        <button
          onClick={() => onOpen(item)}
