...
import { ArrowRight, ExternalLink, GraduationCap, Building } from "lucide-react";
...
export default function StudentCard({ item, onOpen, rank }: Props) {
  ...
  return (
    <div className="group flex flex-col justify-between rounded-3xl border border-ink/10 bg-white p-6 shadow-xs transition hover:-translate-y-1 hover:shadow-xl">
      <div>
          ...
          <div>
            {rank <= 3 && (
              <span className="mb-2 inline-block rounded-full bg-gold/15 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-terracotta">
                ⭐ Top Scholarship #{rank}
              </span>
            )}
            <h3 className="font-display text-lg font-bold leading-snug text-ink group-hover:text-teal-dark transition">
              {scholarship.shortName}
            </h3>
            <p className="mt-1 flex items-center gap-1 text-xs font-medium text-ink/50">
              <Building size={12} /> {scholarship.ministry}
            </p>
          </div>
        ...
        {/* Tagline */}
        <p className="mt-3 text-sm leading-relaxed text-ink/65 line-clamp-2">
          {scholarship.tagline}
        </p>

        {/* Scholarship Grant Amount Badge */}
        <div className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-teal/10 px-3 py-1 text-xs font-bold text-teal-dark">
          <GraduationCap size={14} /> {scholarship.amountLabel}
        </div>
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {reasons.slice(0, 2).map((r, i) => (
            <span
              key={i}
              className="rounded-full bg-black/5 px-2.5 py-0.5 text-[11px] font-medium text-ink/75"
            >
              ✓ {r}
            </span>
          ))}
